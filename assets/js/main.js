...
function win(user, comp) {
    if (gameState === true) {
        ...
        result_h1.textContent = `${capitalizeWords(user)} beats ${capitalizeWords(comp)}. You win !`
...
function lost(user, comp) {
    if (gameState === true) {
        ...
        result_h1.textContent = `${capitalizeWords(user)} loses to ${capitalizeWords(comp)}. You lost!`
        if (computerScore === winningScore) {
            result_h1.textContent = 'You lost the game...'
            gameState = false;
        }
    }
};
...
function draw(user, comp) {
    if(gameState === true) {
        result_h1.textContent = `${capitalizeWords(user)} is even to ${capitalizeWords(comp)}. Its a draw!`
    }
